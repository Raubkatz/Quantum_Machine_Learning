...
# Load and preprocess the Iris dataset, all features of these datasets are continuous no additional preprocessing required
if data_nr == 0:
    data_sk = load_iris()
    data_name = "iris"
elif data_nr == 1:
...
start = datetime.now()
random_search = RandomizedSearchCV(CatBoostClassifier(verbose=0), param_grid, n_iter=n_random_picks, cv=5, random_state=42, verbose=10)
...
gen_out_txt = "CatBoost_RandomizedSearchCV for the " + str(data_name) + " dataset with n_iter=" + str(n_random_picks) + "\n"
...
#write CV stats file
InfoPath = "./CatBoost_RandomizedSearchCV_" + data_name + "_" + str(n_random_picks) + ".txt"
