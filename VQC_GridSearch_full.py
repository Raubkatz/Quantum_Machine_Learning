...
This program provides a wrapper for Qiskit Machine Learning's Variational Quantum Classifier (VQC) and demonstrates how to use scikit-learn's GridSearchCV to find a suitable set of hyperparameters for the VQC model.
"""
import numpy as np
...
start = datetime.now()
grid_search = GridSearchCV(VQCWrapper(), param_grid, cv=cv, verbose=10)
grid_search.fit(X_train, y_train_onehot)

# Test the best model
print("Best parameters:", grid_search.best_params_)
test_score = grid_search.score(X_test, y_test_onehot)
...
gen_out_txt = "VQC_GridSearchCV for the " + str(data_name) + " dataset" + "\n"
...
#write CV stats file
InfoPath = "./VQC_GridSearchCV_" + data_name + ".txt"
